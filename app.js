(function () {
  ...
  const App = {
    ...
    renderDashboard() {
      ...
      const original = this.data.questions.filter(q => q.origin.toLowerCase().includes('original') || q.origin.toLowerCase().includes('instructor')).length;
      return `
        <section class="hero">
          <div>
            <div class="eyebrow">Principles of Finance · TCH 302 · v0.4</div>
            <h1>Learn the source.<br/>Then master the decision.</h1>
            <p class="lead">All seven course modules now have detailed lessons. Lecture content stays primary; instructor Q&A, tutorials, spreadsheets, and uploaded textbooks are layered underneath with explicit source labels.</p>
        ...
        <section class="section"><div class="section-head"><div><div class="eyebrow">v0.4 exam-ready depth</div><h2>Built from your course sources</h2></div></div>
          <div class="grid grid-4">${this.statCard(Object.keys(this.data.lessons).length,'Detailed lessons')}${this.statCard(this.data.formulas.length,'Verified formulas')}${this.statCard(this.data.questions.length,'Practice questions')}${this.statCard(original,'Original / instructor-linked questions')}</div>
        </section>
    ...
    renderModule(id) {
      ...
      return `<div class="page-head"><div><div class="eyebrow">Module ${String(m.order).padStart(2,'0')}</div><h1>${esc(m.name)}</h1><div class="vi">${this.vi(m.vi)}</div><p class="lead" style="margin-top:10px">Primary source: ${esc(m.primarySource)}</p></div><div><span class="badge ${m.coverage.toLowerCase()}">${m.coverage} SOURCE COVERAGE</span><div class="action-row"><button class="btn primary" onclick="App.resetPractice('${m.id}');App.go('practice')">Practice chapter · ${totalQ} Q</button></div></div></div>
        ...
        <section class="section"><div class="section-head"><div><div class="eyebrow">Lessons</div><h2>Deep theory + targeted practice</h2></div></div><div class="grid grid-2">${lessonCards}</div></section>`;
    },
    ...
    renderPractice() {
      ...
      const originButtons=`<button class="btn sm ${this.practice.mode==='original'?'primary':''}" onclick="App.resetPractice('original')">Original / Instructor-linked</button><button class="btn sm ${this.practice.mode==='generated'?'primary':''}" onclick="App.resetPractice('generated')">Generated Practice</button><button class="btn sm ${this.practice.mode==='mistakes'?'primary':''}" onclick="App.resetPractice('mistakes')">My mistakes</button>`;
    ...
    renderQuestionBank() {
      ...
      const filtered=this.data.questions.filter(q=>
        ...
        (f.origin==='all'||(f.origin==='original'?/original|instructor/i.test(q.origin):!(/original|instructor/i.test(q.origin)))) &&
    ...
    renderExam() {
      if(!this.exam)return `<div class="exam-shell"><div class="page-head"><div><div class="eyebrow">Exam Mode</div><h1>English-only test conditions</h1><p class="lead">No Vietnamese translations, hints, sources, or immediate explanations during the test. Review appears only after submission.</p></div></div><div class="card pad"><h2>Source-grounded mixed exam</h2><p class="muted">20 questions · 25 minutes · all seven modules · mixed conceptual and numerical items.</p><button class="btn primary" onclick="App.startExam()">Start exam</button></div></div>`;
    ...
    // Exam
    startExam(){const ids=shuffle([...this.data.questions]).slice(0,Math.min(20,this.data.questions.length)).map(q=>q.id);this.exam={ids,index:0,answers:{},marked:[],remaining:1500,submitted:false,lastTick:Date.now()};this.render();},
